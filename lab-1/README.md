...
### Local audio playback (optional)
The helper `mdl.lab1.play_song()` converts ABC notation to audio using two system executables: `abc2midi` (from `abcmidi`) and `timidity`.

- Google Colab (Linux): `apt-get install -y abcmidi timidity`
- macOS (Homebrew): `brew install abcmidi timidity` (then restart the notebook kernel)
